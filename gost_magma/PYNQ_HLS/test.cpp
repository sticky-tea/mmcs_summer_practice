...
#define SIZEOF_DATA_O_E 24
#define SIZEOF_SYNCHRO 8
...
uint8_t  Imitta[8];
uint8_t  Data_E[SIZEOF_DATA_O_E];
uint8_t  Synchro[8];
uint8_t Data_out[SIZEOF_DATA_O_E];

int main(int argc, char *argv[])
{
    memset(Imitta,_GOST_Def_Byte,8);
    GOST_Imitta(Data_O, Imitta, SIZEOF_DATA_O_E,Gost_Table,GOST_Key_d);
    if (memcmp(Imitta,Imitta_Et,8))
    {
...
//Simple replacement
    memcpy(Data_E,Data_O,SIZEOF_DATA_O_E);


    GOST_Encrypt_SR(Data_E,SIZEOF_DATA_O_E,_GOST_Mode_Encrypt, Data_out);
    if (memcmp(Data_C_S_Et,Data_out,SIZEOF_DATA_O_E))
    {
    	for(int i = 0; i < 24; i++)
    		printf("%hhx ", Data_E[i]);
    	printf("\n");
    ...
    GOST_Encrypt_SR(Data_E,SIZEOF_DATA_O_E,_GOST_Mode_Decrypt, Data_out);
    if (memcmp(Data_O,Data_out,SIZEOF_DATA_O_E))
    {
...
//Gamma
    memcpy(Data_E,Data_O,SIZEOF_DATA_O_E);
    memcpy(Synchro,Synchro_Et,SIZEOF_SYNCHRO);
    GOST_Crypt_G_PS(Synchro,Gost_Table,GOST_Key_d);//Decrypt Synchro acording to standart
    GOST_Crypt_G_Data(Data_E,SIZEOF_DATA_O_E,Synchro,Gost_Table,GOST_Key_d);
    if (memcmp(Data_E,Data_C_G_Et,SIZEOF_DATA_O_E))
    {
    ...
    GOST_Crypt_G_PS(Synchro,Gost_Table,GOST_Key_d);//Decrypt Synchro acording to standart
    GOST_Crypt_G_Data(Data_E,SIZEOF_DATA_O_E,Synchro,Gost_Table,GOST_Key_d);
    if (memcmp(Data_O,Data_E,SIZEOF_DATA_O_E))
    {
...
//Gamma with feedback
...
#endif
    GOST_Crypt_GF_Data(Data_E,SIZEOF_DATA_O_E,Synchro,_GOST_Mode_Encrypt,Gost_Table,GOST_Key_d);
    if (memcmp(Data_E,Data_C_GF_Et,SIZEOF_DATA_O_E))
    {
    ...
    GOST_Crypt_GF_Prepare_S(Synchro);
#endif
    GOST_Crypt_GF_Data(Data_E,SIZEOF_DATA_O_E,Synchro,_GOST_Mode_Decrypt,Gost_Table,GOST_Key_d);
    if (memcmp(Data_O,Data_E,SIZEOF_DATA_O_E))
    {
